...
**Inspiration:** Named after the Indian classical music raaga "Maand," this framework embodies the art of orchestrating and harmonizing job management across a network of agents.

**Objective:** Maand is a static job orchestration framework designed to efficiently manage and execute tasks across multiple agents listed in the `agents.json` file. It coordinates job execution on virtual machines (VMs) within a cluster, ensuring seamless and effective job management.
...
5. **Job Management (`jobs` folder):**
   ...
   - Each job folder must include:
     - `manifest.json`: Defines the `roles` attribute, which is an array of strings indicating which roles the job targets.
...
6. **Rolling Upgrades:**
   - Maand supports rolling upgrades. Each job can include a `commands` folder with a `run.sh` file. Maand executes this script before and after applying changes to an agent to ensure a smooth upgrade process, waiting for successful execution before moving to the next agent.
...
7. **Executing Adhoc Commands (`command.sh`):**
   ...
   - Maand executes run commands under the agent context.
...
Maand is a powerful job orchestration framework that efficiently manages job execution across a cluster of agents. It ensures that agents are correctly configured and validated within the cluster while facilitating secure and flexible job management. With support for various job types, rolling upgrades, and adhoc command execution, Maand optimizes job orchestration, enhancing overall system efficiency and security.
