...
def run_command(agent_ip):
    ...
    if filtered_jobs:
        command_helper.command_remote(f"python /opt/agent/bin/runner.py start --jobs {filtered_jobs}", env=agent_env)
    else:
        command_helper.command_remote(f"python /opt/agent/bin/runner.py start", env=agent_env)
